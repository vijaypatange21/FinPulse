...
const navItems = [
  { key: 'dashboard', label: 'Dashboard', icon: LayoutDashboard, to: '/borrower/dashboard' },
  { key: 'loans', label: 'My Loans', icon: CreditCard, to: '/borrower/loans' },
  { key: 'apply', label: 'Apply for Loan', icon: PlusCircle, to: '/loan-application' },
  { key: 'health-score', label: 'Health Score', icon: Activity, to: '/borrower/health-score' },
  { key: 'transactions', label: 'Transactions', icon: Receipt, to: '/borrower/transactions' },
  { key: 'documents', label: 'Documents / KYC', icon: FileCheck, to: '/borrower/upload' },
  { key: 'recommendations', label: 'AI Wellness', icon: Sparkles, to: '/recommendations' },
  { key: 'find-lender', label: 'Find Lender', icon: Building2, to: '/borrower/find-lender' },
];

export const BorrowerLayout = ({ activeSection, title, children }) => {
      ...
      >
        ...
        {collapsed ? (
          ...
          <div className="h-20 px-5 flex items-center justify-between border-b border-[var(--border-subtle)]">
            <Logo to="/borrower/dashboard" subtitle="Borrower Portal" size="md" />
          ...
          >
            ...
            {!collapsed && (
              <div className="overflow-hidden flex-1">
                ...
                <p className="text-[10px] text-[var(--text-secondary)] truncate">Verified Borrower</p>
              </div>
            )}
          </div>
        ...
        <header className="h-20 glass-chrome rounded-[32px] px-8 flex items-center justify-between z-10 shrink-0 mb-4">
          ...
          <div className="flex items-center gap-3">
            <div className="hidden sm:flex items-center gap-2 bg-[var(--bg-surface-raised)] border border-[var(--border-subtle)] px-3.5 py-1.5 rounded-full text-xs text-[var(--text-secondary)]">
              <span className="w-2 h-2 rounded-full bg-[var(--status-positive)] animate-pulse" />
